...
Segundo, buscamos los strings vacíos que existen en el dataset y sustituirlos con 'Desconocido' para que queden más claro.
```{r message= FALSE, warning=FALSE}
...
grid.arrange(plot_mean_fare, plot_mean_age, ncol=2)
```

Con las visualizaciones anteriores, podemos observar que existe mucha diferencia de la probalibidad de supervivencia por género, clase, edad. Para comprobar nuestras suposiciones, realizamos los contrastes de hipótesis. 
...
auc(r)
```
Con los resultados del AUROC, podemos decir que el modelo discrimina de forma excelente.
